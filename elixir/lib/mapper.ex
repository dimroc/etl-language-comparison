defmodule Mapper do
  ...
  def listen_for_children(count) when count <= 0 do
  end

  def listen_for_children(count) do
    receive do
      :ok -> listen_for_children(count - 1)
    end
  end

  def generate_output_files(input_files, output_dir) do
    File.mkdir_p! output_dir
    Enum.map(input_files, fn f -> Path.join(output_dir, Path.basename f) end)
  end

  def generate_input_files(input_dir) do
    File.ls!(input_dir)
